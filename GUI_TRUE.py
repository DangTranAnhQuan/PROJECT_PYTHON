...
def load_data():
        ...
        df_details = pd.read_csv("d:/Python/DoAn/projectFull/Details.csv")
        df_orders = pd.read_csv("d:/Python/DoAn/projectFull/Orders.csv")
        df_merged = pd.merge(df_details, df_orders, on="Order ID")
        df_sorted = df_merged.sort_values(by="Order ID")
        df_sorted.to_csv("d:/Python/DoAn/projectFull/onlinesales_sorted.csv", index=False)
